...
int main (void) {
    struct conjunto *a; 			//primeiro conjunto (pares 2 até 20)
    struct conjunto *b; 			//segundo conjunto (1 até 20)
    struct conjunto *u; 			//conjunto união entre a e b
    struct conjunto *i; 			//conjunto intersecção entre a e b
    struct conjunto *d; 			//conjunto diferença entre a e b (a-b)
    struct conjunto *d1;			//conjunto diferença entre b e a (b-a) 
    struct conjunto *copia;			//conjunto copia de b
    struct conjunto *sub;			//subconjunto de b
    int k; 							//contador 
    int ex;							//verifica conjunto cheio
	
	srand(0);	

    // Teste para conjunto vazio 
    printf ("---> Teste 1\n");
   	//verifica se o conjunto não é NULL ao criar
    if (!(a = cria_cjt(TAM))) {
        printf ("falha na criacao do conjunto\n");
        return 1;
    }
    //verifica se o conjunto que acabou de criar esta vazio
    //que é como deve ser
    if (! vazio_cjt(a)) {
    ...
    printf ("---> Teste 2\n");
    for(k = 1; k <= 10; k++){
    	//insere em a os numeros pares,de 2 até 20
        ex = insere_cjt(a, 2*k);
        if(ex == 0){
    ...
    //ERRO O LOOP ACIMA VAI APENAS ATÉ 19
    imprime_cjt(b);
    printf("deve ter impresso um subconjunto de tamanho 20\n");
	//FIM DO ERRO
